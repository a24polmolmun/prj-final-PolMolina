...
export class JustificantsManagerService {
    ...
    async afegirJustificant(nouJustificant: Partial<Justificant>) {
        try {
            const creat = await this.apiManager.post<Justificant>('/justificants', nouJustificant);
            this.justificants.update(actuals => [...actuals, creat]);
            return creat;
        } catch (err) {
    ...
    async actualitzarJustificant(id: number, dadesActualitzades: Partial<Justificant>) {
        try {
            const update = await this.apiManager.put<Justificant>(`/justificants/${id}`, dadesActualitzades);
            this.justificants.update(actuals => actuals.map(j => j.id === id ? update : j));
            return update;
        } catch (err) {
    ...
    async esborrarJustificant(id: number) {
        try {
            ...
            this.justificants.update(actuals => actuals.filter(j => j.id !== id));
            return true;
        } catch (err) {
